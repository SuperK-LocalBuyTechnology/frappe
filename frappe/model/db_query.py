...
class DatabaseQuery(object):
    ...
    def execute(self, fields=None, filters=None, or_filters=None,
        ...
        if not ignore_permissions and \
            ...
            frappe.flags.error_message = _(
                'Insufficient Permission for {0}').format(frappe.bold(self.doctype))
    ...
    def prepare_args(self):
        ...
        if self.grouped_or_conditions:
            self.conditions.append("({0})".format(
                " or ".join(self.grouped_or_conditions)))
    ...
    def sanitize_fields(self):
        ...
        def _raise_exception():
            frappe.throw(
                _('Use of sub-query or function is restricted'), frappe.DataError)
    ...
    def build_conditions(self):
        ...
        self.build_filter_conditions(
            self.or_filters, self.grouped_or_conditions)
    ...
    def prepare_filter_condition(self, f):
        ...
        if f.operator.lower() in additional_filters_config:
            f.update(get_additional_filter_field(
                additional_filters_config, f, f.value))
        ...
        if f.operator.lower() in ('ancestors of', 'descendants of', 'not ancestors of', 'not descendants of'):
            ...
            if frappe.get_meta(f.doctype).get_field(f.fieldname) is not None:
                ref_doctype = frappe.get_meta(
                    f.doctype).get_field(f.fieldname).options
            ...
            value = [frappe.db.escape(
                (v.name or '').strip(), percent=False) for v in result]
            if len(value):
            ...
            fallback = "''"
            value = [frappe.db.escape(
                (v or '').strip(), percent=False) for v in values]
            if len(value):
                value = "({0})".format(", ".join(value))
            else:
                value = "('')"
        else:
            df = frappe.get_meta(f.doctype).get(
    ...
    def build_match_conditions(self, as_condition=True):
        ...
        if (not meta.istable and
            ...
            only_if_shared = True
            if not self.shared:
                frappe.throw(_("No permission to read {0}").format(
                    self.doctype), frappe.PermissionError)
            else:
        ...
        if as_condition:
            ...
            if doctype_conditions:
                conditions += (' and ' +
                               doctype_conditions) if conditions else doctype_conditions
    ...
    def add_user_permissions(self, user_permissions):
        ...
        # IMPLEMENT LATER FOR THE CHILD FIELDS
        # doctype_table_fields = meta.get_table_fields()
        ...
        # doctype_table_fields.append(dict(
        #         options=self.doctype,
        #         fieldname='name',
        # ))
        ...
        # NOT NEEDED RIGHT NOW (MAYBE LATER) --- FYI NOT WORKING PROPERLY
        # for df in doctype_table_fields:
        #     if df.get('ignore_user_permissions'):
        #         continue

        #     child_meta = frappe.get_meta(df.get('options'))

        #     childtable_link_fields = []
        #     childtable_link_fields = child_meta.get_link_fields()

        #     childtable_link_fields.append(dict(
        #             options=df.get('options'),
        #             fieldname='name',
        #     ))

        #     for child_df in childtable_link_fields:
        #         child_user_permission_values = user_permissions.get(child_df.get('options'), {})

        #         if child_user_permission_values:
        #             docs = []
        #             if frappe.get_system_settings("apply_strict_user_permissions"):
        #                 condition = ""
        #             else:
        #                 # IF IMPLEMENTED LATER, THIS PART IS NOT WORKING (HAVE TO JOIN CHILD TABLE WITH THE PARENT TABLE)
        #                 tname = ('`tab' + df.get('options') + '`')
        #                 self.append_table(tname)

        #                 empty_value_condition = "ifnull(`tab{doctype}`.`{fieldname}`, '')=''".format(
        #                         doctype=df.get('options'), fieldname=child_df.get('fieldname')
        #                 )
        #                 condition = empty_value_condition + " or "

        #             for permission in child_user_permission_values:
        #                 if permission.get('is_child_table'):
        #                     if not permission.get('applicable_for'):
        #                         docs.append(permission.get('doc'))

        #                     elif child_df.get('fieldname') == 'name' and self.reference_doctype:
        #                         if permission.get('applicable_for') == self.reference_doctype:
        #                             if not permission.get('field_to_restrict'):
        #                                 docs.append(permission.get('doc'))

        #                             if permission.get('field_to_restrict') and permission.get('table_fieldname') and permission.get('field_to_restrict') == child_df.get('fieldname'):
        #                                 docs.append(permission.get('doc'))
        ...
        #             if docs:
        #                 condition += "`tab{doctype}`.`{fieldname}` in ({values})".format(
        #                         doctype=df.get('options'),
        #                         fieldname=child_df.get('fieldname'),
        #                         values=", ".join(
        #                                 [(frappe.db.escape(doc, percent=False)) for doc in docs])
        #                         )

        #                 match_conditions.append("({condition})".format(condition=condition))
        #                 match_filters[child_df.get('options')] = docs
    ...
    def get_permission_query_conditions(self):
        conditions = []
        condition_methods = frappe.get_hooks(
            "permission_query_conditions", {}).get(self.doctype, [])
        ...
        permision_script_name = get_server_script_map().get(
            "permission_query", {}).get(self.doctype)
        ...
        if self.order_by:
            ...
            if not group_function_without_group_by:
                ...
                if meta.sort_field and ',' in meta.sort_field:
                    ...
                    sort_order = (
                        meta.sort_field and meta.sort_order) or 'desc'
                ...
                if meta.is_submittable:
                    args.order_by = "`tab{0}`.docstatus asc, {1}".format(
                        self.doctype, args.order_by)
...
def get_date_range(operator, value):
    ...
    timespan = period_map[operator] + ' ' + \
        timespan_map[value] if operator != 'timespan' else value
