...
def print_has_permission_check_logs(func):
    def inner(*args, **kwargs):
        ...
        self_perm_check = True if not kwargs.get(
            'user') else kwargs.get('user') == frappe.session.user
        raise_exception = False if kwargs.get(
            'raise_exception') == False else True

        # print only if access denied
        # and if user is checking his own permission
        if not result and self_perm_check and raise_exception:
            msgprint(('<br>').join(frappe.flags.get(
                'has_permission_check_logs', [])))
...
def has_permission(doctype, ptype="read", doc=None, verbose=False, user=None, raise_exception=True):
    ...
    if doc:
        ...
        if not perm:
            push_perm_check_log(_('User {0} does not have doctype access via role permission for document {1}').format(
...
def get_doc_permissions(doc, user=None, ptype=None):
    ...
    permissions = copy.deepcopy(get_role_permissions(
        meta, user=user, is_owner=is_user_owner()))
...
def get_role_permissions(doctype_meta, user=None, is_owner=None):
    ...
    if isinstance(doctype_meta, string_types):
        # assuming doctype name was passed
    ...
    if not frappe.local.role_permissions.get(cache_key):
        ...
        has_if_owner_enabled = any(p.get('if_owner', 0)
...
def has_controller_permissions(doc, ptype, user=None):
    ...
    for method in reversed(methods):
        controller_permission = frappe.call(
            frappe.get_attr(method), doc=doc, ptype=ptype, user=user)
...
def get_all_perms(role):
    ...
    custom_perms = frappe.get_all(
        'Custom DocPerm', fields='*', filters=dict(role=role))
...
def get_roles(user=None, with_standard=True):
    ...
    if not with_standard:
        roles = filter(lambda x: x not in [
            'All', 'Guest', 'Administrator'], roles)
...
def add_user_permission(doctype, name, user, ignore_permissions=False, applicable_for=None,
    ...
    if not user_permission_exists(user, doctype, name, applicable_for):
        if not frappe.db.exists(doctype, name):
            frappe.throw(_("{0} {1} not found").format(
                _(doctype), name), frappe.DoesNotExistError)
...
def clear_user_permissions_for_doctype(doctype, user=None):
    ...
    if user:
        filters['user'] = user
    user_permissions_for_doctype = frappe.db.get_all(
        'User Permission', filters=filters)
    ...
    if not ("System Manager" in frappe.get_roles() or has_permission(doctype, "import")):
        if raise_exception:
            raise frappe.PermissionError(
                "You are not allowed to import: {doctype}".format(doctype=doctype))
        else:
            return False
    return True
...
def reset_perms(doctype):
    ...
    frappe.db.sql(
        """delete from `tabCustom DocPerm` where parent=%s""", doctype)
